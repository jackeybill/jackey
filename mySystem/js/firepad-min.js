...
("Firepad", function () {
	...
	a.utils.makeEventEmitter = function (a, b) {
		...
		a.prototype.validateEventType_ = function (a) {
			if (this.allowedEvents_) {
				...
				if (!b)
					throw new Error('Unknown event "' + a + '"')
			}
		}
	},
	...
	a.utils.stopEvent = function (b) {
		a.utils.preventDefault(b),
		a.utils.stopPropagation(b)
	},
	...
	a.utils.stringEndsWith = function (a, b) {
		for (var c = "string" == typeof b ? [b] : b, d = 0; d < c.length; d++) {
			var b = c[d];
			if (-1 !== a.indexOf(b, a.length - b.length))
				return !0
		}
		return !1
	},
	...
	a.TextOperation = function () {
		...
		return b.prototype.equals = function (a) {
			if (this.baseLength !== a.baseLength)
				return !1;
			if (this.targetLength !== a.targetLength)
				return !1;
			if (this.ops.length !== a.ops.length)
				return !1;
			for (var b = 0; b < this.ops.length; b++)
				if (!this.ops[b].equals(a.ops[b]))
					return !1;
			return !0
		},
		b.prototype.retain = function (a, b) {
			...
			return c && c.isRetain() && c.attributesEqual(b) ? c.chars += a : this.ops.push(new e("retain", a, b)),
			this
		},
		b.prototype.insert = function (a, b) {
			...
			if ("" === a)
				return this;
			b = b || {},
			this.targetLength += a.length;
			var c = this.ops.length > 0 ? this.ops[this.ops.length - 1] : null,
			d = this.ops.length > 1 ? this.ops[this.ops.length - 2] : null;
			return c && c.isInsert() && c.attributesEqual(b) ? c.text += a : c && c.isDelete() ? d && d.isInsert() && d.attributesEqual(b) ? d.text += a : (this.ops[this.ops.length - 1] = new e("insert", a, b), this.ops.push(c)) : this.ops.push(new e("insert", a, b)),
			this
		},
		b.prototype["delete"] = function (a) {
			...
			return b && b.isDelete() ? b.chars += a : this.ops.push(new e("delete", a)),
			this
		},
		...
		b.prototype.clone = function () {
			for (var a = new b, c = 0; c < this.ops.length; c++)
				this.ops[c].isRetain() ? a.retain(this.ops[c].chars, this.ops[c].attributes) : this.ops[c].isInsert() ? a.insert(this.ops[c].text, this.ops[c].attributes) : a["delete"](this.ops[c].chars);
			return a
		},
		b.prototype.toString = function () {
			var a = Array.prototype.map || function (a) {
				for (var b = this, c = [], d = 0, e = b.length; e > d; d++)
					c[d] = a(b[d]);
				return c
			};
		...
		b.prototype.toJSON = function () {
			for (var a = [], b = 0; b < this.ops.length; b++)
				this.ops[b].hasEmptyAttributes() || a.push(this.ops[b].attributes), "retain" === this.ops[b].type ? a.push(this.ops[b].chars) : "insert" === this.ops[b].type ? a.push(this.ops[b].text) : "delete" === this.ops[b].type && a.push(-this.ops[b].chars);
			return 0 === a.length && a.push(0),
			a
		},
		b.fromJSON = function (a) {
			for (var c = new b, d = 0, e = a.length; e > d; d++) {
				...
				"number" == typeof g ? g > 0 ? c.retain(g, h) : c["delete"](-g) : (f.assert("string" == typeof g), c.insert(g, h))
			}
			return c
		},
		b.prototype.apply = function (a, b, c) {
			...
			for (var e, g, h = [], i = 0, j = 0, k = this.ops, l = 0, m = k.length; m > l; l++) {
				...
				if (n.isRetain()) {
					...
					for (h[i++] = n.text, e = 0; e < n.text.length; e++) {
						var q = {};
						for (g in n.attributes)
							q[g] = n.attributes[g], f.assert(q[g] !== !1);
						c.push(q)
					}
				else
					j += n.chars
			...
			var r = h.join("");
			return f.assert(r.length === c.length),
			r
		},
		...
		b.prototype.compose = function (a) {
			...
			for (var e, f = new b, g = d.clone().ops, h = a.clone().ops, i = 0, j = 0, k = g[i++], l = h[j++]; ; ) {
				if ("undefined" == typeof k && "undefined" == typeof l)
					break;
				if (k && k.isDelete())
					f["delete"](k.chars), k = g[i++];
				else if (l && l.isInsert())
					f.insert(l.text, l.attributes), l = h[j++];
				else {
					...
					if (k.isRetain() && l.isRetain())
						e = c(k.attributes, l.attributes), k.chars > l.chars ? (f.retain(l.chars, e), k.chars -= l.chars, l = h[j++]) : k.chars === l.chars ? (f.retain(k.chars, e), k = g[i++], l = h[j++]) : (f.retain(k.chars, e), l.chars -= k.chars, k = g[i++]);
					else if (k.isInsert() && l.isDelete())
						k.text.length > l.chars ? (k.text = k.text.slice(l.chars), l = h[j++]) : k.text.length === l.chars ? (k = g[i++], l = h[j++]) : (l.chars -= k.text.length, k = g[i++]);
					else if (k.isInsert() && l.isRetain())
						e = c(k.attributes, l.attributes, !0), k.text.length > l.chars ? (f.insert(k.text.slice(0, l.chars), e), k.text = k.text.slice(l.chars), l = h[j++]) : k.text.length === l.chars ? (f.insert(k.text, e), k = g[i++], l = h[j++]) : (f.insert(k.text, e), l.chars -= k.text.length, k = g[i++]);
					else {
		...
		b.transformAttributes = function (a, b) {
			...
			for (c in g) {
				var h = a[c],
				i = b[c];
				f.assert(null != h || null != i),
				null == h ? e[c] = i : null == i ? d[c] = h : h === i || (d[c] = h)
			}
			return [d, e]
		},
		b.transform = function (a, c) {
			if (a.baseLength !== c.baseLength)
				throw new Error("Both operations have to have the same base length");
			for (var d = new b, e = new b, f = a.clone().ops, g = c.clone().ops, h = 0, i = 0, j = f[h++], k = g[i++]; ; ) {
				if ("undefined" == typeof j && "undefined" == typeof k)
					break;
				if (j && j.isInsert())
					d.insert(j.text, j.attributes), e.retain(j.text.length), j = f[h++];
				else if (k && k.isInsert())
					d.retain(k.text.length), e.insert(k.text, k.attributes), k = g[i++];
				else {
					...
					if (j.isRetain() && k.isRetain()) {
						...
						j.chars > k.chars ? (l = k.chars, j.chars -= k.chars, k = g[i++]) : j.chars === k.chars ? (l = k.chars, j = f[h++], k = g[i++]) : (l = j.chars, k.chars -= j.chars, j = f[h++]),
						d.retain(l, m[0]),
						e.retain(l, m[1])
					} else if (j.isDelete() && k.isDelete())
						j.chars > k.chars ? (j.chars -= k.chars, k = g[i++]) : j.chars === k.chars ? (j = f[h++], k = g[i++]) : (k.chars -= j.chars, j = f[h++]);
					else if (j.isDelete() && k.isRetain())
						j.chars > k.chars ? (l = k.chars, j.chars -= k.chars, k = g[i++]) : j.chars === k.chars ? (l = k.chars, j = f[h++], k = g[i++]) : (l = j.chars, k.chars -= j.chars, j = f[h++]), d["delete"](l);
					else {
						if (!j.isRetain() || !k.isDelete())
							throw new Error("The two operations aren't compatible");
						j.chars > k.chars ? (l = k.chars, j.chars -= k.chars, k = g[i++]) : j.chars === k.chars ? (l = j.chars, j = f[h++], k = g[i++]) : (l = j.chars, k.chars -= j.chars, j = f[h++]),
						e["delete"](l)
					}
				}
			}
	...
	a.AnnotationList = function () {
		function b(a, b) {
			if (!a)
				throw new Error("AnnotationList assertion failed" + (b ? ": " + b : ""))
		}
		function c(a, b) {
			this.pos = a,
		...
		return e.prototype.insertAnnotatedSpan = function (a, c) {
			this.wrapOperation_(new g(a.pos, 0), function (d, e) {
				...
				if (e) {
					...
					var i = new f(0, h);
					return i.next = new f(a.pos - d, e.annotation),
					i.next.next = g,
		...
		e.prototype.removeSpan = function (a) {
			0 !== a.length && this.wrapOperation_(a, function (c, d) {
				...
				return i > 0 && (g.next = new f(i, d.annotation)),
				e.next
			})
		},
		e.prototype.updateSpan = function (a, c) {
			0 !== a.length && this.wrapOperation_(a, function (d, e) {
				...
				for (b(k < e.length), k > 0 && (i.next = new f(k, e.annotation), i = i.next, j += i.length); null !== e && a.end() >= d + e.length; ) {
					var l = d + e.length - j;
					i.next = new f(l, c(e.annotation, l)),
					i = i.next,
					d += e.length,
					e = e.next,
					j = d
				...
				var m = a.end() - j;
				return m > 0 && (b(m < e.length), i.next = new f(m, c(e.annotation, m)), i = i.next, j += i.length, i.next = new f(d + e.length - j, e.annotation)),
				g.next
			})
		},
		e.prototype.wrapOperation_ = function (a, b) {
			...
			if (j) {
				...
				var m;
				for (i.pred && i.pred.annotation.equals(j.annotation) ? (k = !0, j.length += i.pred.length, i.beforePred.next = j, m = i.predPos) : (i.beforeStart.next = j, m = i.startPos); j.next; )
					h.push(new d(m, j)), m += j.length, j = j.next;
				i.succ && i.succ.annotation.equals(j.annotation) ? (j.length += i.succ.length, l = !0, j.next = i.succ.next) : j.next = e,
				h.push(new d(m, j))
			} else
				i.pred && i.succ && i.pred.annotation.equals(i.succ.annotation) ? (k = !0, l = !0, j = new f(i.pred.length + i.succ.length, i.pred.annotation), i.beforePred.next = j, j.next = i.succ.next, h.push(new d(i.startPos - i.pred.length, j))) : i.beforeStart.next = e;
			k && g.push(new c(i.predPos, i.pred));
			for (var n = i.startPos, o = i.start; null !== o; )
				g.push(new c(n, o)), n += o.length, o = o.next;
			l && g.push(new c(n, i.succ)),
			this.changeHandler_(g, h)
		},
		e.prototype.getAffectedNodes_ = function (a) {
			...
			for (b.startPos = f, b.start = 0 === a.length && a.pos === f ? null : e, b.beforeStart = d, f === a.pos && f > 0 ? (b.pred = d, b.predPos = f - d.length, b.beforePred = c) : b.pred = null; null !== e && a.end() > f; )
				f += e.length, d = e, e = e.next;
			if (a.end() > f)
				throw new Error("Span end exceeds the bounds of the AnnotationList.");
			return b.end = 0 === a.length && a.end() === f ? null : d,
			b.succ = f === a.end() ? e : null,
			b
		},
		e.prototype.mergeNodesWithSameAnnotations_ = function (a) {
			if (a)
				for (var b = null, c = a; c; )
					b && b.annotation.equals(c.annotation) ? (b.length += c.length, b.next = c.next) : b = c, c = c.next
		},
		e.prototype.forEach = function (a) {
			for (var b = this.head_.next; null !== b; )
				a(b.length, b.annotation, b.attachedObject), b = b.next
		},
		e.prototype.getAnnotatedSpansForPos = function (a) {
			...
			var f = [];
			return b === a && e && f.push(new c(b - e.length, e)),
			d && f.push(new c(b, d)),
			f
		},
	...
	();
	var a = a || {};
	a.FirebaseAdapter = function () {
		function b(a, b, c) {
			this.ref_ = a,
			this.ready_ = !1,
			this.firebaseCallbacks_ = [],
			this.zombie_ = !1,
			this.document_ = new f,
			this.revision_ = 0,
			this.pendingReceivedRevisions_ = {},
			this.setUserId(b),
			this.setColor(c);
			var d = this;
			this.firebaseOn_(a.root().child(".info/connected"), "value", function (a) {
		...
		function e(a) {
			c(a.length > 0 && a[0] === i[a.length + 8]);
			for (var b = 0, d = 1; d < a.length; d++)
				b *= i.length, b += i.indexOf(a[d]);
			return b
		}
		var f = a.TextOperation,
		g = a.utils,
		h = 100;
		g.makeEventEmitter(b, ["ready", "cursor", "operation", "ack", "retry"]),
		...
		b.prototype.setUserId = function (a) {
			this.userRef_ && (this.userRef_.child("cursor").remove(), this.userRef_.child("cursor").onDisconnect().cancel(), this.userRef_.child("color").remove(), this.userRef_.child("color").onDisconnect().cancel()),
			this.userId_ = a,
			this.userRef_ = this.ref_.child("users").child(a),
			this.initializeUserData_()
		},
		b.prototype.isHistoryEmpty = function () {
			return c(this.ready_, "Not ready yet."),
			0 === this.revision_
		},
		b.prototype.sendOperation = function (a) {
			function b(a, c) {
				e.ref_.child("history").child(a).transaction(function (a) {
					return null === a ? c : void 0
				}, function (d) {
					if (d) {
						if ("disconnect" !== d.message)
							throw g.log("Transaction failure!", d), d;
						e.sent_ && e.sent_.id === a && setTimeout(function () {
							b(a, c)
						}, 0)
					}
				}, !1)
			...
			c(this.document_.targetLength === a.baseLength, "sendOperation() called with invalid operation.");
			var f = d(this.revision_);
			this.sent_ = {
				id : f,
				op : a
			},
		...
		b.prototype.sendCursor = function (a) {
			this.userRef_.child("cursor").set(a),
			this.cursor_ = a
		},
		b.prototype.setColor = function (a) {
			this.userRef_.child("color").set(a),
			this.color_ = a
		},
		...
		b.prototype.initializeUserData_ = function () {
		...
		b.prototype.monitorCursors_ = function () {
			function a(a) {
				var b = a.name(),
				d = a.val();
		...
		b.prototype.monitorHistory_ = function () {
			var a = this;
			this.ref_.child("checkpoint").once("value", function (b) {
				if (!a.zombie_) {
					...
					d = b.child("o").val(),
					f = b.child("a").val();
					null != d && null != c && null !== f ? (a.pendingReceivedRevisions_[c] = {
							o : d,
							a : f
						}, a.checkpointRevision_ = e(c), a.monitorHistoryStartingAt_(a.checkpointRevision_ + 1)) : (a.checkpointRevision_ = 0, a.monitorHistoryStartingAt_(a.checkpointRevision_))
				}
			})
		},
		b.prototype.monitorHistoryStartingAt_ = function (a) {
			var b = this.ref_.child("history").startAt(null, d(a)),
			c = this;
			setTimeout(function () {
				c.firebaseOn_(b, "child_added", function (a) {
					var b = a.name();
					c.pendingReceivedRevisions_[b] = a.val(),
					c.ready_ && c.handlePendingReceivedRevisions_()
				}),
		...
		b.prototype.handleInitialRevisions_ = function () {
			c(!this.ready_, "Should not be called multiple times."),
			this.revision_ = this.checkpointRevision_;
			for (var a = d(this.revision_), b = this.pendingReceivedRevisions_; null != b[a]; ) {
				var e = this.parseRevision_(b[a]);
				e ? this.document_ = this.document_.compose(e.operation) : g.log("Invalid operation.", this.ref_.toString(), a, b[a]),
				delete b[a],
				this.revision_++,
				a = d(this.revision_)
			}
		...
		b.prototype.handlePendingReceivedRevisions_ = function () {
			for (var a = this.pendingReceivedRevisions_, b = d(this.revision_), c = !1; null != a[b]; ) {
				this.revision_++;
				var e = this.parseRevision_(a[b]);
				e ? (this.document_ = this.document_.compose(e.operation), this.sent_ && b === this.sent_.id ? this.sent_.op.equals(e.operation) && e.author === this.userId_ ? (this.revision_ % h === 0 && this.saveCheckpoint_(), this.sent_ = null, this.trigger("ack")) : (c = !0, this.trigger("operation", e.operation)) : this.trigger("operation", e.operation)) : g.log("Invalid operation.", this.ref_.toString(), b, a[b]),
				delete a[b],
				b = d(this.revision_)
			}
			c && (this.sent_ = null, this.trigger("retry"))
		},
		b.prototype.parseRevision_ = function (a) {
			if ("object" != typeof a)
				return null;
			if ("string" != typeof a.a || "object" != typeof a.o)
				return null;
			var b = null;
			try {
				b = f.fromJSON(a.o)
		...
		b.prototype.saveCheckpoint_ = function () {
			this.ref_.child("checkpoint").set({
				a : this.userId_,
				o : this.document_.toJSON(),
				id : d(this.revision_ - 1)
			})
		},
		b.prototype.firebaseOn_ = function (a, b, c, d) {
		...
		b.prototype.firebaseOff_ = function (a, b, c, d) {
			a.off(b, c, d);
		...
		b.prototype.removeFirebaseCallbacks_ = function () {
	...
	();
	var a = a || {};
	a.RichTextToolbar = function () {
		function b(a) {
			this.imageInsertionUI = a,
			this.element_ = this.makeElement_()
		}
		var c = a.utils;
		return c.makeEventEmitter(b, ["bold", "italic", "underline", "strike", "font", "font-size", "color", "left", "center", "right", "unordered-list", "ordered-list", "todo-list", "indent-increase", "indent-decrease", "undo", "redo", "insert-image"]),
		b.prototype.element = function () {
			return this.element_
		},
		b.prototype.makeButton_ = function (a, b) {
			var d = this;
			b = b || a;
			var e = c.elt("a", [c.elt("span", "", {
							"class" : "firepad-tb-" + b
						})], {
					"class" : "firepad-btn"
				});
			return c.on(e, "click", c.stopEventAnd(function () {
					d.trigger(a)
				})),
			e
		},
		b.prototype.makeElement_ = function () {
			var a = this,
			b = this.makeFontDropdown_(),
			d = this.makeFontSizeDropdown_(),
			...
			f = [c.elt("div", [b], {
					...
					"class" : "firepad-btn-group"
				}), c.elt("div", [a.makeButton_("bold"), a.makeButton_("italic"), a.makeButton_("underline"), a.makeButton_("strike", "strikethrough")], {
					"class" : "firepad-btn-group"
				}), c.elt("div", [a.makeButton_("unordered-list", "list-2"), a.makeButton_("ordered-list", "numbered-list"), a.makeButton_("todo-list", "list")], {
					"class" : "firepad-btn-group"
				}), c.elt("div", [a.makeButton_("indent-decrease"), a.makeButton_("indent-increase")], {
					"class" : "firepad-btn-group"
				}), c.elt("div", [a.makeButton_("left", "paragraph-left"), a.makeButton_("center", "paragraph-center"), a.makeButton_("right", "paragraph-right")], {
					"class" : "firepad-btn-group"
				})];
			a.imageInsertionUI && f.push(c.elt("div", [a.makeButton_("insert-image")], {
					"class" : "firepad-btn-group"
				}));
			var g = c.elt("div", f, {
		...
		b.prototype.makeFontDropdown_ = function () {
			for (var a = ["Arial", "Comic Sans MS", "Courier New", "Impact", "Times New Roman", "Verdana"], b = [], d = 0; d < a.length; d++) {
				var e = c.elt("span", a[d]);
				e.setAttribute("style", "font-family:" + a[d]),
				b.push({
					content : e,
					value : a[d]
				})
		...
		b.prototype.makeFontSizeDropdown_ = function () {
			for (var a = [9, 10, 12, 14, 18, 24, 32, 42], b = [], d = 0; d < a.length; d++) {
				var e = c.elt("span", a[d].toString());
				e.setAttribute("style", "font-size:" + a[d] + "px; line-height:" + (a[d] - 6) + "px;"),
				b.push({
					content : e,
					value : a[d]
				})
		...
		b.prototype.makeColorDropdown_ = function () {
			for (var a = ["black", "red", "green", "blue", "yellow", "cyan", "magenta", "grey"], b = [], d = 0; d < a.length; d++) {
				var e = c.elt("div");
				e.className = "firepad-color-dropdown-item",
				e.setAttribute("style", "background-color:" + a[d]),
				b.push({
					content : e,
					value : a[d]
				})
		...
		b.prototype.makeDropdown_ = function (a, b, d, e) {
			function f() {
				l || (k.style.display = "block", c.on(document, "click", g, !0), l = !0)
			}
			function g() {
				l && (k.style.display = "", c.off(document, "click", g, !0), l = !1),
				m = !0,
				setTimeout(function () {
					m = !1
				}, 0)
			...
			function h(a, d) {
				...
				var f = c.elt("a", [a]);
				c.on(f, "click", c.stopEventAnd(function () {
						g(),
						i.trigger(b, d + e)
					})),
				k.appendChild(f)
			...
			j.appendChild(k);
			for (var l = !1, m = !1, n = 0; n < d.length; n++) {
				var o = d[n].content,
				p = d[n].value;
				h(o, p)
			}
			return c.on(j, "click", c.stopEventAnd(function () {
					m || f()
				})),
			j
		},
	...
	a.WrappedOperation = function () {
		...
		a.prototype.invert = function () {
			var b = this.meta;
			return new a(this.wrapped.invert.apply(this.wrapped, arguments), b && "object" == typeof b && "function" == typeof b.invert ? b.invert.apply(b, arguments) : b)
		},
		a.prototype.compose = function (b) {
			return new a(this.wrapped.compose(b.wrapped), c(this.meta, b.meta))
		},
		a.transform = function (b, c) {
			var e = b.wrapped.constructor.transform,
			f = e(b.wrapped, c.wrapped);
			return [new a(f[0], d(b.meta, c.wrapped)), new a(f[1], d(c.meta, b.wrapped))]
		},
		a
	}
	();
	...
	a.UndoManager = function () {
		...
		function b(a, b) {
			for (var c = [], d = b.constructor, e = a.length - 1; e >= 0; e--) {
				...
				"function" == typeof f[0].isNoop && f[0].isNoop() || c.push(f[0]),
				b = f[1]
			}
			return c.reverse()
		}
		...
		return a.prototype.add = function (a, b) {
			if (this.state === d)
				this.redoStack.push(a), this.dontCompose = !0;
			else if (this.state === e)
				this.undoStack.push(a), this.dontCompose = !0;
			else {
				var c = this.undoStack;
				!this.dontCompose && b && c.length > 0 ? c.push(a.compose(c.pop())) : (c.push(a), c.length > this.maxItems && c.shift()),
				this.dontCompose = !1,
				this.redoStack = []
			}
		},
		a.prototype.transform = function (a) {
			this.undoStack = b(this.undoStack, a),
			this.redoStack = b(this.redoStack, a)
		},
		a.prototype.performUndo = function (a) {
			if (this.state = d, 0 === this.undoStack.length)
				throw new Error("undo not possible");
			a(this.undoStack.pop()),
			this.state = c
		},
		a.prototype.performRedo = function (a) {
			if (this.state = e, 0 === this.redoStack.length)
	...
	a.Client = function () {
		...
		a.prototype.sendOperation = function () {
			throw new Error("sendOperation must be defined in child class")
		},
		a.prototype.applyOperation = function () {
			throw new Error("applyOperation must be defined in child class")
		},
		a.Synchronized = b,
		b.prototype.applyClient = function (a, b) {
			return a.sendOperation(b),
			new c(b)
		},
		b.prototype.applyServer = function (a, b) {
			return a.applyOperation(b),
			this
		},
		b.prototype.serverAck = function () {
			throw new Error("There is no pending operation.")
		},
		b.prototype.serverRetry = function () {
			throw new Error("There is no pending operation.")
		};
		var e = new b;
		return a.AwaitingConfirm = c,
		...
		c.prototype.applyServer = function (a, b) {
			...
			return a.applyOperation(d[1]),
			new c(d[0])
		},
		c.prototype.serverAck = function () {
			return e
		},
		c.prototype.serverRetry = function (a) {
			return a.sendOperation(this.outstanding),
			this
		},
		...
		d.prototype.serverAck = function (a) {
			return a.sendOperation(this.buffer),
			new c(this.buffer)
		},
		a
	}
	...
	a.EditorClient = function () {
		...
		d.prototype.onChange = function (a, c) {
			...
			this.undoManager.add(new j(c, g), e),
			this.applyClient(a)
		},
		...
		d.prototype.onCursorActivity = function () {
			var a = this.cursor;
			this.updateCursor(),
			!this.focused || a && this.cursor.equals(a) || this.sendCursor(this.cursor)
		},
		...
		d.prototype.sendOperation = function (a) {
			this.serverAdapter.sendOperation(a),
			this.emitStatus()
		},
		d.prototype.applyOperation = function (a) {
			this.editorAdapter.applyOperation(a),
			this.updateCursor(),
			this.undoManager.transform(new j(a, null))
		},
	...
	a.ACEAdapter = b = function () {
		function b(a) {
			this.ace = a,
			this.onCursorActivity = c(this.onCursorActivity, this),
			this.onBlur = c(this.onBlur, this),
			this.onChange = c(this.onChange, this),
		...
		return b.prototype.ignoreChanges = !1,
		b.prototype.grabDocumentState = function () {
			return this.lastDocLines = this.aceDoc.getAllLines(),
			this.lastCursorRange = this.aceSession.selection.getRange()
		},
		b.prototype.detach = function () {
			return this.ace.removeListener("change", this.onChange),
		...
		b.prototype.onChange = function (a) {
			var b;
			return this.ignoreChanges ? void 0 : (b = this.operationFromACEChange(a), this.trigger.apply(this, ["change"].concat(d.call(b))), this.grabDocumentState())
		},
		...
		b.prototype.operationFromACEChange = function (b) {
			var c,
			d,
			e,
			f,
			...
			k;
			return d = b.data,
			"insertLines" === (j = d.action) || "removeLines" === j ? (i = d.lines.join("\n") + "\n", c = d.action.replace("Lines", "")) : (i = d.text.replace(this.aceDoc.getNewLineCharacter(), "\n"), c = d.action.replace("Text", "")),
			h = this.indexFromPos(d.range.start),
			g = this.lastDocLines.join("\n").length - h,
			"remove" === c && (g -= i.length),
			f = (new a.TextOperation).retain(h).insert(i).retain(g),
			e = (new a.TextOperation).retain(h)["delete"](i).retain(g),
			"remove" === c && (k = [e, f], f = k[0], e = k[1]),
			[f, e]
		},
		b.prototype.applyOperationToACE = function (a) {
			var b,
			c,
			d,
			e,
			...
			i,
			j;
			for (null == this.aceRange && (this.aceRange = (null != (i = ace.require) ? i : require)("ace/range").Range), c = 0, j = a.ops, g = 0, h = j.length; h > g; g++)
				d = j[g], d.isRetain() ? c += d.chars : d.isInsert() ? (this.aceDoc.insert(this.posFromIndex(c), d.text), c += d.text.length) : d.isDelete() && (b = this.posFromIndex(c), f = this.posFromIndex(c + d.chars), e = this.aceRange.fromPoints(b, f), this.aceDoc.remove(e));
			return this.grabDocumentState()
		},
		b.prototype.posFromIndex = function (a) {
			var b,
			c,
			d,
			e,
		...
		b.prototype.indexFromPos = function (a, b) {
			var c,
			d,
			e,
			f;
			for (null == b && (b = this.lastDocLines), d = 0, c = e = 0, f = a.row; f >= 0 ? f > e : e > f; c = f >= 0 ? ++e : --e)
		...
		b.prototype.getCursor = function () {
			...
			try {
				...
				b = h;
				try {
					e = this.indexFromPos(this.lastCursorRange.start),
					d = this.indexFromPos(this.lastCursorRange.end)
				} catch (h) {
					c = h,
		...
		b.prototype.setCursor = function (a) {
			...
			d,
			e;
			return null == this.aceRange && (this.aceRange = (null != (d = ace.require) ? d : require)("ace/range").Range),
			c = this.posFromIndex(a.position),
			b = this.posFromIndex(a.selectionEnd),
			a.position > a.selectionEnd && (e = [b, c], c = e[0], b = e[1]),
		...
		b.prototype.setOtherCursor = function (a, b, c) {
			...
			j,
			k = this;
			return null == this.otherCursors && (this.otherCursors = {}),
			f = this.otherCursors[c],
			f && (f.start.detach(), f.end.detach(), this.aceSession.removeMarker(f.id)),
			i = this.posFromIndex(a.position),
			g = this.posFromIndex(a.selectionEnd),
			a.selectionEnd < a.position && (j = [g, i], i = j[0], g = j[1]),
			d = "other-client-selection-" + b.replace("#", ""),
			h = a.position === a.selectionEnd,
			h && (g.column += 1, d = d.replace("selection", "cursor")),
			e = "." + d + " {\n  position: absolute;\n  background-color: " + (h ? "transparent" : b) + ";\n  border-left: 2px solid " + b + ";\n}",
			this.addStyleRule(e),
			this.otherCursors[c] = f = new this.aceRange(i.row, i.column, g.row, g.column),
			f.start = this.aceDoc.createAnchor(f.start),
			f.end = this.aceDoc.createAnchor(f.end),
			f.id = this.aceSession.addMarker(f, d, "text"), {
				clear : function () {
					return f.start.detach(),
					f.end.detach(),
					k.aceSession.removeMarker(f.id)
				}
			}
		},
		b.prototype.addStyleRule = function (a) {
			var b;
			if ("undefined" != typeof document && null !== document && (this.addedStyleRules || (this.addedStyleRules = {}, b = document.createElement("style"), document.documentElement.getElementsByTagName("head")[0].appendChild(b), this.addedStyleSheet = b.sheet), !this.addedStyleRules[a]))
		...
		b.prototype.trigger = function () {
			...
			null != (c = this.callbacks) && null != (e = c[b]) ? e.apply(this, a) : void 0
		},
		b.prototype.applyOperation = function (a) {
			return a.isNoop() || (this.ignoreChanges = !0),
			this.applyOperationToACE(a),
	...
	a.EntityManager = function () {
		...
		b.prototype.exportToElement = function (a) {
			...
			return b.setAttribute("data-firepad-entity", a.type),
			b
		},
		...
		b.prototype.tryRenderToElement_ = function (b, c, d) {
			...
			if (this.entities_[e] && this.entities_[e][c]) {
				var g = this.entities_[e][c](f, d);
				if (g) {
					if ("string" == typeof g) {
						var h = document.createElement("div");
						return h.innerHTML = g,
						h.childNodes[0]
					}
					if ("object" == typeof g)
						return a.utils.assert("undefined" != typeof g.nodeType, "Error rendering " + e + " entity.  render() function must return an html string or a DOM element."), g
				}
			}
		},
	...
	a.Entity = function () {
		...
		b.fromAttributes = function (a) {
			...
			f = {};
			for (var g in a)
				0 === g.indexOf(e) && (f[g.substr(e.length)] = a[g]);
			return new b(c, f)
		},
		b
	}
	...
	a.RichTextCodeMirror = function () {
		function b(a, b, c) {
			...
			/^4\./.test(CodeMirror.version) ? this.codeMirror.on("changes", this.onCodeMirrorChange_) : this.codeMirror.on("change", this.onCodeMirrorChange_),
		...
		var r = "";
		b.LineSentinelCharacter = r;
		var s = "";
		return b.EntitySentinelCharacter = s,
		b.prototype.detach = function () {
		...
		b.prototype.toggleAttribute = function (a, b) {
			...
			if (this.emptySelection_()) {
				...
				d[a] === c ? delete d[a] : d[a] = c,
				this.currentAttributes_ = d
			} else {
		...
		b.prototype.setAttribute = function (a, b) {
			...
			if (this.emptySelection_()) {
				var d = this.getCurrentAttributes_();
				b === !1 ? delete d[a] : d[a] = b,
				this.currentAttributes_ = d
			} else
				this.updateTextAttributes(c.indexFromPos(c.getCursor("start")), c.indexFromPos(c.getCursor("end")), function (c) {
		...
		b.prototype.updateTextAttributes = function (a, b, c, d, e) {
			...
			this.annotationList_.updateSpan(new k(a, b - a), function (a, b) {
				...
				var n = {},
				o = {};
				return j.computeChangedAttributes_(a.attributes, k, n, o),
				h(n) || f.push({
					start : i,
					end : i + b,
					attributes : n,
		...
		b.prototype.computeChangedAttributes_ = function (a, b, c, d) {
			...
			for (e in b)
				f[e] = !0;
			for (e in f)
				e in b ? e in a ? a[e] !== b[e] && (c[e] = b[e], d[e] = a[e]) : (c[e] = b[e], d[e] = !1) : (c[e] = !1, d[e] = a[e])
		},
		b.prototype.toggleLineAttribute = function (a, b) {
			var c,
			d = this.getCurrentLineAttributes_();
			c = a in d && d[a] === b ? !1 : b,
			this.setLineAttribute(a, c)
		},
		...
		b.prototype.updateLineAttributesForSelection = function (a) {
			...
			f > e && h && f--,
			this.updateLineAttributes(e, f, a)
		},
		b.prototype.updateLineAttributes = function (a, b, c) {
			for (var d = a; b >= d; d++) {
				...
				if (e[0] !== r) {
					...
					g[m.LINE_SENTINEL] = !0,
					c(g),
					this.insertText(f, r, g)
				} else
					this.updateTextAttributes(f, f + 1, c, null, !0)
			}
		},
		...
		b.prototype.onAnnotationsChanged_ = function (a, b) {
			...
			for (var n in d)
				this.dirtyLines_.push(this.codeMirror.getLineHandle(Number(n))), this.queueLineMarking_()
		},
		b.prototype.tryToUpdateEntitiesInPlace = function (a, b) {
			for (var c = a.length; c--; )
				for (var d = a[c], e = b.length; e--; ) {
					var f = b[e];
					if (d.pos == f.pos && d.annotation.attributes.ent && d.annotation.attributes.ent == f.annotation.attributes.ent) {
						var g = f.annotation.attributes.ent;
						if (this.entityManager_.entitySupportsUpdate(g)) {
							a.splice(c, 1),
							b.splice(e, 1);
							var h = d.getAttachedObject();
							h.update(f.annotation.attributes),
							f.attachObject(h)
						}
					}
				}
		...
		b.prototype.getClassNameForAttributes_ = function (b) {
			...
			for (var d in b) {
				var e = b[d];
				if (d === m.LINE_SENTINEL)
					a.utils.assert(e === !0, "LINE_SENTINEL attribute should be true if it exists.");
				else {
		...
		b.prototype.onCodeMirrorChange_ = function (a, b) {
			...
			for (var d = this.convertCoordinateSystemForChanges_(b), e = [], f = 0; f < d.length; f++) {
				...
				if (j.length > 0) {
					var r;
					"+input" === h.origin || "paste" === h.origin ? r = this.currentAttributes_ || {}

					 : m in this.outstandingChanges_ ? (r = this.outstandingChanges_[m].attributes, m = this.outstandingChanges_[m].origOrigin, delete this.outstandingChanges_[m]) : r = {},
			...
			this.markLineSentinelCharactersForChanges_(b),
			e.length > 0 && this.trigger("change", this, e)
		},
		...
		b.prototype.markLineSentinelCharactersForChanges_ = function (a) {
			for (var b = Number.MAX_VALUE, c = -1, d = 0; d < a.length; d++) { {
				...
				e.text.length > 1 ? (b = Math.min(b, f), c = Math.max(c, f + e.text.length - 1)) : e.text[0].indexOf(r) >= 0 && (b = Math.min(b, f), c = Math.max(c, f))
			}
			c = Math.min(c, this.codeMirror.lineCount() - 1),
			this.markLineSentinelCharactersForChangedLines_(b, c)
		},
		b.prototype.markLineSentinelCharactersForChangedLines_ = function (a, b) {
			if (a < Number.MAX_VALUE)
				for (; a > 0 && this.lineIsListItemOrIndented_(a - 1); )
					a--;
			if (b > -1)
				for (var c = this.codeMirror.lineCount(); c > b + 1 && this.lineIsListItemOrIndented_(b + 1); )
					b++;
			for (var d = [], e = this.codeMirror, f = a; b >= f; f++) {
				...
				if (e.removeLineClass(h, "text", ".*"), g.length > 0)
					for (var i = g.indexOf(r); i >= 0; ) {
						...
						this.markLineSentinelCharacters_(f, j, i, d),
						i = g.indexOf(r, i)
					}
				else
					d = []
		...
		b.prototype.markLineSentinelCharacters_ = function (a, b, c, d) {
			...
			if (0 === b) {
				...
				"" !== l && this.codeMirror.addLineClass(a, "text", l),
				d = d.slice(0, k + 1)
			}
			var n = {
				inclusiveLeft : !0,
		...
		b.prototype.toggleTodo = function (a) {
			...
			c = m.LIST_TYPE,
			d = this.getCurrentLineAttributes_();
			c in d && ("t" === d[c] || "tc" === d[c]) ? "t" === d[c] ? b = "tc" : "tc" === d[c] && (b = a ? "t" : !1) : b = "t",
		...
		b.prototype.makeTodoListElement_ = function (a, b) {
			...
			e = this;
			return l.on(d, "click", l.stopEventAnd(function () {
		...
		b.prototype.getCurrentAttributes_ = function () {
			return this.currentAttributes_ || this.updateCurrentAttributes_(),
		...
		b.prototype.updateCurrentAttributes_ = function () {
			...
			for (var i in h)
				"l" !== i && "lt" !== i && "li" !== i && 0 !== i.indexOf(m.ENTITY_SENTINEL) && (this.currentAttributes_[i] = h[i])
		},
		b.prototype.getCurrentLineAttributes_ = function () {
			...
			c = a.getCursor("head"),
			d = c.line;
			return 0 === c.ch && b.line < c.line && d--,
			this.getLineAttributes_(d)
		},
		b.prototype.getLineAttributes_ = function (b) {
			...
			if (d.length > 0 && d[0] === r) {
				...
				for (var g in f[0].annotation.attributes)
					c[g] = f[0].annotation.attributes[g]
			}
			return c
		},
		b.prototype.clearAnnotations_ = function () {
			this.annotationList_.updateSpan(new k(0, this.end()), function () {
				return new g({})
			})
		},
		b.prototype.newline = function () {
			...
			if (this.emptySelection_()) {
				...
				e && 1 === a.getLine(c).length ? this.updateLineAttributes(c, c, function (a) {
					delete a[m.LIST_TYPE],
					delete a[m.LINE_INDENT]
				}) : (a.replaceSelection("\n", "end", "+input"), this.updateLineAttributes(c + 1, c + 1, function (a) {
						for (var f in d)
							a[f] = d[f];
						"tc" === e && (a[m.LIST_TYPE] = "t"),
						b.trigger("newLine", {
							line : c + 1,
							attr : a
						})
		...
		b.prototype.deleteLeft = function () {
			...
			f && d ? this.updateLineAttributes(b.line, b.line, function (a) {
				delete a[m.LIST_TYPE],
				delete a[m.LINE_INDENT]
			}) : f && e && e > 0 ? this.updateLineAttributes(b.line, b.line, function (a) {
				a[m.LINE_INDENT]--
			}) : a.deleteH(-1, "char")
		},
		...
		g.prototype.equals = function (a) {
			...
			for (b in this.attributes)
				if (a.attributes[b] !== this.attributes[b])
					return !1;
	...
	a.RichTextCodeMirrorAdapter = function () {
		...
		b.operationFromCodeMirrorChanges = function (a, b) {
			for (var c = e(b), d = (new i).retain(c), f = (new i).retain(c), g = a.length - 1; g >= 0; g--) {
				var h = a[g],
				j = h.start,
				k = c - j - h.text.length;
				d = (new i).retain(j)["delete"](h.removed.length).insert(h.text, h.attributes).retain(k).compose(d),
				f = f.compose((new i).retain(j)["delete"](h.text.length).insert(h.removed, h.removedAttributes).retain(k)),
				c += h.removed.length - h.text.length
			}
			return [d, f]
		},
		b.operationFromAttributesChanges = function (a, b) {
			for (var c = e(b), d = new i, g = new i, h = 0, j = a.length - 1; j >= 0; j--) {
				var k = a[j],
				l = k.start - h;
				f(l >= 0),
				d.retain(l),
				g.retain(l);
				var m = k.end - k.start;
				d.retain(m, k.attributes),
				g.retain(m, k.attributesInverse),
				h = k.start + m
			}
			return d.retain(c - h),
			g.retain(c - h),
			...
			for (var c = a.ops, d = 0, e = 0, f = c.length; f > e; e++) {
				...
				g.isRetain() ? (h(g.attributes) || b.updateTextAttributes(d, d + g.chars, function (a) {
						for (var b in g.attributes)
							g.attributes[b] === !1 ? delete a[b] : a[b] = g.attributes[b]
			...
			if ("string" == typeof b && b.match(/^#[a-fA-F0-9]{3,6}$/)) {
				...
				if ("object" == typeof a && "number" == typeof a.position && "number" == typeof a.selectionEnd && !(a.position < 0 || a.position > e || a.selectionEnd < 0 || a.selectionEnd > e)) {
					if (a.position === a.selectionEnd) {
						...
						return g.className = "other-client",
						...
						g.style.height = .9 * (f.bottom - f.top) + "px",
						g.setAttribute("data-clientid", c),
						g.style.zIndex = 0,
						this.cm.setBookmark(d, {
							widget : g,
							insertLeft : !0
						})
					}
					...
					var j,
					k;
					return a.selectionEnd > a.position ? (j = d, k = this.cm.posFromIndex(a.selectionEnd)) : (j = this.cm.posFromIndex(a.selectionEnd), k = d),
					this.cm.markText(j, k, {
						className : h
					})
				}
			}
		...
		b.prototype.invertOperation = function (a) {
			for (var b, c, d = 0, e = this.rtcm.codeMirror, f = new i, g = 0; g < a.wrapped.ops.length; g++) {
				var k = a.wrapped.ops[g];
				if (k.isRetain())
					if (h(k.attributes))
						f.retain(k.chars), d += k.chars;
					else
						for (b = this.rtcm.getAttributeSpans(d, d + k.chars), c = 0; c < b.length; c++) {
							var l = {};
							for (var m in k.attributes) {
								var n = k.attributes[m],
								o = b[c].attributes[m];
								n === !1 ? o && (l[m] = o) : n !== o && (l[m] = o || !1)
							}
							f.retain(b[c].length, l),
							d += b[c].length
						}
				else if (k.isInsert())
					f["delete"](k.text.length);
				else if (k.isDelete()) {
					var p = e.getRange(e.posFromIndex(d), e.posFromIndex(d + k.chars));
					b = this.rtcm.getAttributeSpans(d, d + k.chars);
					var q = 0;
					for (c = 0; c < b.length; c++)
						f.insert(p.substr(q, b[c].length), b[c].attributes), q += b[c].length;
					d += k.chars
				}
			}
			return new j(f, a.meta.invert())
		},
		b
	}
	();
	...
	a.Formatting = function () {
		...
		return b.prototype.cloneWithNewAttribute_ = function (a, c) {
			...
			for (var e in this.attributes)
				d[e] = this.attributes[e];
			return c === !1 ? delete d[a] : d[a] = c,
			new b(d)
		},
	...
	a.LineFormatting = function () {
		...
		b.prototype.listItem = function (b) {
			return a.utils.assert(b === !1 || "u" === b || "o" === b || "t" === b || "tc" === b),
	...
	a.ParseHtml = function () {
		...
		function e(b, c, d) {
			if (b.nodeType === l.ELEMENT_NODE) {
				var e = k.fromElement(b);
				if (e)
					return void d.currentLine.push(new a.Text(i.EntitySentinelCharacter, new a.Formatting(e.toAttributes())))
			}
			switch (b.nodeType) {
			case l.TEXT_NODE:
				var m = b.nodeValue.replace(/[ \n\t]+/g, " ");
				d.currentLine.push(a.Text(m, c.textFormatting));
				break;
			case l.ELEMENT_NODE:
				var n = b.getAttribute("style") || "";
				switch (c = h(c, n), b.nodeName.toLowerCase()) {
				case "div":
				case "h1":
				case "h2":
				case "h3":
				...
				case "s":
					...
					break;
				case "font":
					var o = b.getAttribute("face"),
					p = b.getAttribute("color"),
					q = parseInt(b.getAttribute("size"));
					o && (c = c.withTextFormatting(c.textFormatting.font(o))),
					p && (c = c.withTextFormatting(c.textFormatting.color(p))),
					q && (c = c.withTextFormatting(c.textFormatting.fontSize(q))),
					f(b, c, d);
					break;
				case "br":
					d.newline(c);
				...
				case "ul":
					...
					var r = "firepad-todo" === b.getAttribute("class") ? j.TODO : j.UNORDERED;
					f(b, c.withListType(r).withIncreasedIndent(), d),
					d.newlineIfNonEmpty(c);
					break;
				case "ol":
					d.newlineIfNonEmptyOrListItem(c),
					f(b, c.withListType(j.ORDERED).withIncreasedIndent(), d),
					d.newlineIfNonEmpty(c);
					break;
				case "li":
					g(b, c, d);
		...
		function f(a, b, c) {
			if (a.hasChildNodes())
				for (var d = 0; d < a.childNodes.length; d++)
					e(a.childNodes[d], b, c)
		}
		function g(a, b, c) {
			c.newlineIfNonEmptyOrListItem(b);
			var d = "firepad-checked" === a.getAttribute("class") ? j.TODOCHECKED : b.listType;
			c.makeListItem(d);
			var e = c.currentLine;
			f(a, b, c),
			(e === c.currentLine || c.currentLine.length > 0) && c.newline(b)
		}
		function h(b, c) {
		...
		var i = a.RichTextCodeMirror,
		j = a.LineFormatting.LIST_TYPE;
		...
		c.prototype.newlineIfNonEmpty = function (a) {
			this.cleanLine_(),
			this.currentLine.length > 0 && this.newline(a)
		},
		c.prototype.newlineIfNonEmptyOrListItem = function (a) {
			this.cleanLine_(),
			(this.currentLine.length > 0 || null !== this.currentLineListItemType) && this.newline(a)
		},
		c.prototype.newline = function (b) {
			this.cleanLine_();
			var c = b.lineFormatting;
			null !== this.currentLineListItemType && (c = c.listItem(this.currentLineListItemType), this.currentLineListItemType = null),
		...
		c.prototype.isListItem = function () {
			return null !== this.currentLineListItemType
		},
	...
	return a.Firepad = function (b) {
		function c(a, b, e) {
			...
			if (q && b instanceof q) {
			...
			} else if (r && b && b.session instanceof r.EditSession) {
				...
				this.codeMirror_ = this.editor_ = new q(b);
			...
			this.options_ = e || {},
			this.getOption("richTextShortcuts", !1) && (q.keyMap.richtext || this.initializeKeyMap_(), this.codeMirror_.setOption("keyMap", "richtext"), this.firepadWrapper_.className += " firepad-richtext"),
			this.imageInsertionUI = this.getOption("imageInsertionUI", !0),
			this.getOption("richTextToolbar", !1) && (this.addToolbar_(), this.firepadWrapper_.className += " firepad-richtext firepad-with-toolbar"),
			this.addPoweredByLogo_(),
			this.codeMirror_ && this.codeMirror_.refresh();
			var n = this.getOption("userId", a.push().name()),
			p = this.getOption("userColor", d(n));
			this.entityManager_ = new m,
			this.registerBuiltinEntities_(),
			this.firebaseAdapter_ = new k(a, n, p),
			this.codeMirror_ ? (this.richTextCodeMirror_ = new h(this.codeMirror_, this.entityManager_, {
						cssPrefix : "firepad-"
					}), this.editorAdapter_ = new g(this.richTextCodeMirror_)) : this.editorAdapter_ = new j(this.ace_),
		...
		o = a.utils,
		p = a.LineFormatting.LIST_TYPE,
		q = b.CodeMirror,
		r = b.ace;
		return o.makeEventEmitter(c),
		c.fromCodeMirror = c,
		c.fromACE = c,
		c.prototype.dispose = function () {
			...
			this.codeMirror_ && "richtext" === this.codeMirror_.getOption("keyMap") && this.codeMirror_.setOption("keyMap", "default"),
		...
		c.prototype.getText = function () {
			return this.assertReady_("getText"),
			this.codeMirror_ ? this.richTextCodeMirror_.getText() : this.ace_.getSession().getDocument().getValue()
		},
		c.prototype.setText = function (a) {
			return this.ace_ ? this.ace_.getSession().getDocument().setValue(a) : (this.codeMirror_.getWrapperElement().setAttribute("style", "display: none"), this.codeMirror_.setValue(""), this.insertText(0, a), this.codeMirror_.getWrapperElement().setAttribute("style", ""), this.codeMirror_.refresh(), void 0)
		},
		...
		c.prototype.insertText = function (b, c) {
			function d(a, c) {
				i.richTextCodeMirror_.insertText(b, a, c || null),
				b += a.length,
				g = "\n" === a[a.length - 1]
			}
			function e(b) {
				if (b instanceof a.Text)
					d(b.text, b.formatting.attributes);
				else {
					if ("string" != typeof b)
						throw console.error("Can't insert into firepad", b), "Can't insert into firepad: " + b;
					d(b)
				}
			}
			function f(a) {
				g && d(h.LineSentinelCharacter, a.formatting.attributes);
				for (var b = 0; b < a.textPieces.length; b++)
					e(a.textPieces[b]);
				d("\n")
			}
			...
			for (var g = 0 === b, i = this, j = 0; j < c.length; j++)
				c[j]instanceof a.Line ? f(c[j]) : e(c[j])
		},
		c.prototype.getOperationForSpan = function (b, c) {
			for (var d = this.richTextCodeMirror_.getRange(b, c), e = this.richTextCodeMirror_.getAttributeSpans(b, c), f = 0, g = new a.TextOperation, h = 0; h < e.length; h++)
				g.insert(d.substr(f, e[h].length), e[h].attributes), f += e[h].length;
			return g
		},
		c.TODO_STYLE = '<style>ul.firepad-todo { list-style: none; margin-left: 0; padding-left: 0; } ul.firepad-todo > li { padding-left: 1em; text-indent: -1em; } ul.firepad-todo > li:before { content: "\\2610"; padding-right: 5px; } ul.firepad-todo > li.firepad-checked:before { content: "\\2611"; padding-right: 5px; }</style>\n',
		...
		c.prototype.getHtmlFromRange = function (b, d) {
			function e(a) {
				return a === p.ORDERED ? "<ol>" : a === p.UNORDERED ? "<ul>" : '<ul class="firepad-todo">'
			}
			function f(a) {
				return a === p.ORDERED ? "</ol>" : "</ul>"
			}
			function g(a, b) {
				return a === b || a === p.TODO && b === p.TODOCHECKED || a === p.TODOCHECKED && b === p.TODO
			}
			var h = null != b && null != d ? this.getOperationForSpan(b, d) : this.getOperationForSpan(0, this.codeMirror_.getValue().length),
			i = "",
			j = !0;
			i += c.EXPORT_HTML_STYLE;
			for (var k = [], l = !1, m = !0, q = !0, r = 0, s = h.ops[r], t = !1; s; ) {
				o.assert(s.isInsert());
				var u = s.attributes;
				if (j) {
					j = !1;
					...
					w = null,
					x = "left";
					n.LINE_SENTINEL in u && (v = u[n.LINE_INDENT] || 0, w = u[n.LIST_TYPE] || null, x = u[n.LINE_ALIGN] || "left"),
					w && (v = v || 1),
					l ? (i += "</li>", l = !1) : m || (q && (i += "<br/>"), i += "</div>"),
					m = !1,
					o.assert(v >= 0, "Indent must not be negative.");
					for (; k.length > v || v === k.length && null !== w && !g(w, k[k.length - 1]); )
						i += f(k.pop());
					for (; k.length < v; ) {
						var y = w || p.UNORDERED;
						t = w == p.TODO || w == p.TODOCHECKED || t,
						i += e(y),
						k.push(y)
					}
				...
				if (n.LINE_SENTINEL in u)
					s = h.ops[++r];
				else if (n.ENTITY_SENTINEL in u) {
					for (var B = 0; B < s.text.length; B++) {
						var C = a.Entity.fromAttributes(u),
						D = this.entityManager_.exportToElement(C);
						i += D.outerHTML
					}
					s = h.ops[++r]
				} else {
					...
					for (var G in u) {
						...
						d,
						H = u[G];
						G === n.BOLD || G === n.ITALIC || G === n.UNDERLINE || G === n.STRIKE ? (o.assert(H === !0), b = d = G) : G === n.FONT_SIZE ? (b = 'span style="font-size: ' + H, b += "string" != typeof H || -1 === H.indexOf("px", H.length - 2) ? 'px"' : '"', d = "span") : G === n.FONT ? (b = 'span style="font-family: ' + H + '"', d = "span") : G === n.COLOR ? (b = 'span style="color: ' + H + '"', d = "span") : G === n.BACKGROUND_COLOR ? (b = 'span style="background-color: ' + H + '"', d = "span") : o.log(!1, "Encountered unknown attribute while rendering html: " + G),
						b && (E += "<" + b + ">"),
						d && (F = "</" + d + ">" + F)
					}
					var I = s.text,
					J = I.indexOf("\n");
					J >= 0 ? (j = !0, s = J < I.length - 1 ? new a.TextOp("insert", I.substr(J + 1), u) : h.ops[++r], I = I.substr(0, J)) : s = h.ops[++r],
					I = I.replace(/  +/g, function (a) {
							return new Array(a.length + 1).join(" ")
						}).replace(/^ /, " ").replace(/ $/, " "),
					I.length > 0 && (q = !1),
					i += E + this.textToHtml_(I) + F
				}
			}
			for (l ? i += "</li>" : m || (q && (i += "&nbsp;"), i += "</div>"); k.length > 0; )
				i += f(k.pop());
			return t && (i = c.TODO_STYLE + i),
			i
		},
		c.EXPORT_HTML_STYLE = "",
		c.prototype.textToHtml_ = function (a) {
		...
		c.prototype.isHistoryEmpty = function () {
			return this.assertReady_("isHistoryEmpty"),
			this.firebaseAdapter_.isHistoryEmpty()
		},
		c.prototype.bold = function () {
			this.richTextCodeMirror_.toggleAttribute(n.BOLD),
			this.codeMirror_.focus()
		},
		c.prototype.italic = function () {
			this.richTextCodeMirror_.toggleAttribute(n.ITALIC),
			this.codeMirror_.focus()
		},
		c.prototype.underline = function () {
			this.richTextCodeMirror_.toggleAttribute(n.UNDERLINE),
			this.codeMirror_.focus()
		},
		c.prototype.strike = function () {
			this.richTextCodeMirror_.toggleAttribute(n.STRIKE),
			this.codeMirror_.focus()
		},
		c.prototype.fontSize = function (a) {
			this.richTextCodeMirror_.setAttribute(n.FONT_SIZE, a),
			this.codeMirror_.focus()
		},
		c.prototype.font = function (a) {
			this.richTextCodeMirror_.setAttribute(n.FONT, a),
			this.codeMirror_.focus()
		},
		c.prototype.color = function (a) {
			this.richTextCodeMirror_.setAttribute(n.COLOR, a),
			this.codeMirror_.focus()
		},
		...
		c.prototype.align = function (a) {
			...
			this.richTextCodeMirror_.setLineAttribute(n.LINE_ALIGN, a),
			this.codeMirror_.focus()
		},
		c.prototype.orderedList = function () {
			this.richTextCodeMirror_.toggleLineAttribute(n.LIST_TYPE, "o"),
			this.codeMirror_.focus()
		},
		c.prototype.unorderedList = function () {
			this.richTextCodeMirror_.toggleLineAttribute(n.LIST_TYPE, "u"),
			this.codeMirror_.focus()
		},
		...
		c.prototype.assertReady_ = function (a) {
			if (!this.ready_)
				throw new Error('You must wait for the "ready" event before calling ' + a + ".");
			if (this.zombie_)
		...
		c.prototype.makeDialog_ = function (a, b) {
			...
			d = function () {
				...
				c.firepadWrapper_.removeChild(a)
			},
			e = function () {
		...
		c.prototype.registerBuiltinEntities_ = function () {
			var a = ["src", "alt", "width", "height", "style", "class"];
			this.registerEntity("img", {
				render : function (a) {
					o.assert(a.src, "image entity should have 'src'!");
					for (var b = ["src", "alt", "width", "height", "style", "class"], c = "<img ", d = 0; d < b.length; d++) {
						var e = b[d];
						e in a && (c += " " + e + '="' + a[e] + '"')
					}
					return c += ">"
